...
void XMLCALL ChapterHtmlSlimParser::characterData(void* userData, const XML_Char* s, const int len) {
  ...
  if (blockWordCount > softFlushThreshold && !self->inRuby) {
    ...
    // makePages() normally applies the block's top margin before laying it out; a soft flush
    // emits lines first, so apply it here (idempotent) or the margin lands mid-paragraph.
...
void ChapterHtmlSlimParser::addLineToPage(std::unique_ptr<TextBlock> line, const uint32_t visibleOffset) {
  ...
  // Line compression can pull advanceY below the glyph box, which clips descenders at the page
  // bottom. Break on the real ink height, but keep advancing by lineHeight.
  ...
  // currentPageNextY > 0: a line taller than the whole viewport must not emit a blank page first.
