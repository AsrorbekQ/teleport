...
available while Teleport is in File Transfer or Calibre Wireless mode.
...
Teleport-only. Backs the Read Later app; the queue lives at
`/apps/readlater/queue.txt` on the SD card.

### `GET /api/readlater`

Returns the queue as `text/plain`, one `url<TAB>title` per line. An empty body
means nothing is queued.

```bash
curl http://crosspoint.local/api/readlater
```

### `POST /api/readlater`

Appends a URL to the queue. The device fetches and caches the page on demand
from the Read Later app, not here.

```bash
curl -X POST \
  -d "url=https://example.com/article" \
  -d "title=Example" \
  http://crosspoint.local/api/readlater
```

| Parameter | Required | Description |
|-----------|----------|-------------|
| `url` | Yes | Must start with `http://` or `https://` |
| `title` | No | Display title; falls back to the URL |

Response: `{"ok":true}`. A URL already in the queue is ignored. Errors are
`400` with `{"ok":false,"error":"missing url"}` or
`{"ok":false,"error":"url must start with http(s)://"}`, and `500` with
`{"ok":false,"error":"write failed"}` if the SD card write fails.
