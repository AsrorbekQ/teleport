...
// Wi-Fi status glyph, drawn beside the header battery: solid arcs when
// associated, dotted when the radio is up but unconnected, slashed when off.
// (x, y) is the glyph's top-left; it occupies wifiIndicatorWidth x 12 px.
constexpr int wifiIndicatorWidth = 12;

void drawWifiIndicator(const GfxRenderer& renderer, const int wifiX, const int wifiY) {
  // Clear the bounding rect to avoid ghosting between states.
...
void BaseTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  if (batteryDetached) {
    ...
    const int16_t reserve =
        static_cast<int16_t>(batteryReserve + wifiIndicatorWidth + tokens.spaceSm + tokens.spaceMd);
    if (batteryLeft) {
  ...
  // Wi-Fi status sits just inside the battery group, on the battery's side of
  // the band; the clock owns the opposite corner.
  const int16_t wifiX = batteryLeft
                            ? static_cast<int16_t>(batteryX + batteryReserve + tokens.spaceSm)
                            : static_cast<int16_t>(batteryX - tokens.spaceSm - wifiIndicatorWidth);
  drawWifiIndicator(renderer, wifiX, band.y + (batteryH - wifiIndicatorWidth) / 2);
}
