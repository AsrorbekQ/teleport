...
// Small variants for the compact menu (13+ entries, 36px rows), which only the
// Teleport app icons ship. The engine's own small icons moved to the SDK Icons
// library, whose bit layout drawIcon can't read, so those stay at 32 - still
// inside a compact row.
const uint8_t* icon24ForName(UIIcon icon) {
  switch (icon) {
    case UIIcon::Clock:
      return Clock24Icon;
    case UIIcon::Dice:
...
void LyraTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = pageStartIndex; i < buttonCount && i < pageStartIndex + pageItems; ++i) {
    ...
    if (rowIcon != nullptr) {
      ...
      int drawIconSize = 24;
      const uint8_t* iconBitmap = menuRowHeight >= 48 ? nullptr : icon24ForName(icon);
      if (iconBitmap == nullptr) {
        drawIconSize = mainMenuIconSize;
        iconBitmap = iconForName(icon);
      }
      if (iconBitmap != nullptr) {
        renderer.drawIcon(iconBitmap, textX, tileRect.y + (menuRowHeight - drawIconSize) / 2, drawIconSize);
