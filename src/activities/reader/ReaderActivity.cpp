...
std::unique_ptr<ReaderActivity> ReaderActivity::create(GfxRenderer& renderer, MappedInputManager& mappedInput,
  ...
  if (FsHelpers::hasXtcExtension(path)) {
    ...
    // .html/.htm go to the text reader, which strips their markup: Read Later caches fetched
    // articles as HTML and the EPUB pipeline needs a container, not a bare file.
...
bool ReaderActivity::handleBackNavigation() {
  return ReaderUtils::handleBackNavigation(mappedInput, activityManager, bookPath.c_str(), {this, [](void* ctx) {
                                             auto* self = static_cast<ReaderActivity*>(ctx);
                                             // Readers opened from an app (Read Later, RSS, Wikipedia, ...) are
                                             // pushed on top of it, so "home" for them means back to that app.
                                             if (self->bookPath.rfind("/apps/", 0) == 0 ||
                                                 self->bookPath.rfind("/websites/", 0) == 0) {
