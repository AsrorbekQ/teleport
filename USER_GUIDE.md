...
Teleport is a fork of [CrossPoint Reader](https://github.com/crosspoint-reader/crosspoint-reader); everything below the Applications section is the CrossPoint reading engine as we ship it.
...
The Home screen is the main entry point to the firmware. From here you can navigate to **[Reading Mode](#4-reading-mode)** with the most recently read book, the **[Applications](#39-applications)**, **[Browse Files](#33-browse-files-screen)**, the **[Library](#34-library-screen)**, **[File Transfer](#35-file-transfer-screen)**, or **[Settings](#36-settings)**. Teleport orders the menu Flashcards, Habits, Briefing, Settings, then the rest, with File Transfer last.
...
- **Check for updates**: Check for Teleport firmware updates over Wi-Fi. Firmware can also be updated without a USB connection by placing a `.bin` file anywhere on the SD card and picking it from **SD Firmware Update** (or by holding Power and Up at boot).
...
Teleport adds a small suite of offline-first applications on top of the reading
engine. They live on the Home screen; each keeps its data under `/apps/<app>/`
on the SD card.

- **Flashcards**: Spaced-repetition study (SM-2) of a vocabulary deck at
  `/apps/flashcards/gre.deck`. Confirm shows the answer; Left/Right choose
  Again, Hard, Good or Easy and Confirm rates the card. The X4 has no
  battery-backed clock, so you advance the study day yourself with **Day -** /
  **Day +**; on the overview screen the side buttons set how many new cards are
  introduced per day.
- **Habits**: Daily check-ins. Up/Down select a habit, Confirm toggles today,
  Right adds a habit, Left opens details with current and best streaks and a
  30-day completion grid (Left again deletes the habit).
- **Briefing**: Date, weather (Open-Meteo), today's Apple Calendar events and
  Reminders (served by Nest over mDNS), habit streaks and flashcards due.
  Up/Down page through it, Confirm refreshes over Wi-Fi. See below for the
  sleep-screen configuration.
- **Read Later**: A queue of web pages pushed from Nest, from the RSS app, or
  by `POST /api/readlater`. Confirm fetches a page over Wi-Fi and opens it,
  Right fetches everything queued, Left removes a link. Up to 40 entries; each
  cached page is capped at 400 KB because the text reader holds the file in RAM.
- **RSS**: Feed reader with an offline cache. Refresh connects, downloads and
  caches; Right on a post sends it to Read Later. Feeds are cut at 512 KB so
  full-text feeds finish on a small heap.
- **Dice & 8-Ball**: Dice, coin, spinner and Magic 8-Ball, inherited from
  CrossPoint Apps.

**Briefing as the sleep screen.** `/apps/briefing/config.txt` is normally
written by Nest, but it is a plain `key=value` file:

| Key | Values | Meaning |
|-----|--------|---------|
| `enabled` | `1` / `0` | Show the briefing as the sleep screen |
| `sleep_refresh` | `never`, `stale` (default), `always` | When to fetch at sleep time |
| `lat`, `lon`, `city` | numbers / text | Location for the weather lookup |
| `tasks_url` | URL | Plain text, one task per line; a `! ` prefix marks overdue |

A sleep-time fetch is skipped below 20% battery and at most one runs every six
hours. Because the device has no battery-backed clock, `stale` only trusts the
cache age after an NTP sync in the same session; otherwise it fetches only when
the cache is empty.

**Offline-first paradigm.** Read Later and RSS never poll in the background.
You press Refresh (or Fetch) to connect, the text is cached to the SD card, and
the radio goes back off so you can read without draining the battery.
